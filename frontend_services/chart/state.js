...
function generateLookbackPeriodsFromRange() {
    const periods = [];
    for (let i = this.regressionSettings.minLookback; 
         i <= this.regressionSettings.maxLookback; 
         i += this.regressionSettings.stepSize) {
        periods.push(i);
    }
    this.regressionSettings.lookbackPeriods = periods;
    return periods;
}