# Port 8005 - WebSocket Heikin Ashi Data Service
import logging
import sys
import os
import asyncio
...
# Logging Setup
def setup_logging():
    log_formatter = logging.Formatter('%(asctime)s - WEBSOCKET_HEIKIN_ASHI - %(levelname)s - %(message)s')
...
def calculate_historical_heikin_ashi(regular_candles: List[Candle]) -> List[HeikinAshiCandle]:
    """Calculate a full series of Heikin Ashi candles from regular OHLC data."""
...
class HeikinAshiLiveCalculator:
    """Calculate live Heikin Ashi candles one at a time by maintaining state."""
    def __init__(self):
        self.prev_ha_open: Optional[float] = None
        self.prev_ha_close: Optional[float] = None

    def initialize_from_history(self, historical_ha_candles: List[HeikinAshiCandle]):
        """Initialize state from historical HA candles."""
    ...
    def _calculate_candle(self, regular_candle: Candle) -> HeikinAshiCandle:
    ...
    def calculate_next_completed(self, completed_regular_candle: Candle) -> HeikinAshiCandle:
        """Calculate the next COMPLETED Heikin Ashi candle and update internal state."""
    ...
    def calculate_current_bar(self, in_progress_regular_candle: Candle) -> HeikinAshiCandle:
        """Calculate the CURRENT, in-progress Heikin Ashi candle without modifying state."""
...
class ConnectionManager:
    """Manage WebSocket connections and subscription groups for Heikin Ashi data."""
    def __init__(self):
    ...
    async def _send_backfill_data(self, websocket: WebSocket, conn_info: ConnectionInfo) -> bool:
        ...
        logger.info(f"Attempting Heikin Ashi backfill for {conn_info.symbol}/{conn_info.interval}")
        try:
            ...
            logger.debug(f"Cache operation: Fetched {len(cached_ticks_str)} cached ticks for {conn_info.symbol}.") # DEBUG: Cache operations
            ...
            logger.debug(f"Data transformation step: Parsed {len(ticks)} ticks from cached data for {conn_info.symbol}.") # DEBUG: Data transformation steps
            if not ticks:
            ...
            # Resample to regular bars first
            resampled_bars = await resample_ticks_to_bars(ticks, conn_info.interval, conn_info.timezone)
            logger.debug(f"Data transformation step: Resampled {len(ticks)} ticks into {len(resampled_bars)} regular bars for {conn_info.symbol}/{conn_info.interval}.") # DEBUG: Data transformation steps
            ...
            logger.debug(f"Data transformation step: Calculated {len(final_bars)} Heikin Ashi bars from regular bars for {conn_info.symbol}/{conn_info.interval}.") # DEBUG: Data transformation steps

            if final_bars:
                payload = [bar.model_dump() for bar in final_bars]
                logger.info(f"Data fetch completion: Sending {len(payload)} Heikin Ashi backfilled bars to client for {conn_info.symbol}/{conn_info.interval}") # INFO: Data fetch completions
                await websocket.send_json(payload)
                logger.info(f"Sent {len(payload)} Heikin Ashi backfilled bars to client for {conn_info.symbol}/{conn_info.interval}")
            else:
                logger.warning(f"Missing data scenario: No Heikin Ashi bars generated for {conn_info.symbol}/{conn_info.interval}. Sending empty backfill.") # WARNING: Missing data scenarios
                await websocket.send_json([])

            return True

        except (WebSocketDisconnect, ConnectionClosed):
            logger.info(f"Could not send Heikin Ashi backfill to {conn_info.symbol}; client disconnected during the process.")
            return True
        except Exception as e:
            logger.error(f"Critical data processing error: An unexpected error occurred sending Heikin Ashi backfill data for {conn_info.symbol}: {e}", exc_info=True) # ERROR: Critical data processing errors
            return False
    ...
    async def add_connection(self, websocket: WebSocket, symbol: str, interval: str, timezone: str) -> bool:
        ...
        logger.info(f"New client connection: Heikin Ashi {symbol}/{interval} from {websocket.client.host}") # INFO: New client connections
        ...
        # Always create Heikin Ashi calculator for this connection
        if resampler_key not in group.heikin_ashi_calculators:
        ...
        if backfill_successful and websocket.client_state == WebSocketState.CONNECTED:
            ...
            logger.info(f"Heikin Ashi connection for {symbol}/{interval} is now live.")
            return True
        else:
            logger.warning(f"Connection retries and fallbacks: Did not add Heikin Ashi {symbol}/{interval} to live group; client disconnected during backfill.") # WARNING: Connection retries and fallbacks
            return False
    ...
    async def remove_connection(self, websocket: WebSocket):
        ...
        if group:
            ...
            logger.info(f"Client disconnected: Removed Heikin Ashi connection for {conn_info.symbol}/{conn_info.interval}. Remaining connections in group: {len(group.connections)}") # INFO: Client disconnections
    ...
    async def _process_tick_for_group(self, group: SubscriptionGroup, tick_data: dict):
        """Process a single raw tick, generate Heikin Ashi data, and send to clients."""
        ...
        for resampler_key, resampler in group.resamplers.items():
            try:
                ...
                ha_calc = group.heikin_ashi_calculators.get(resampler_key)
                completed_ha_bar, current_ha_bar = None, None
                
                if completed_bar and ha_calc:
                    completed_ha_bar = ha_calc.calculate_next_completed(completed_bar)
                    logger.debug(f"Heikin Ashi calculation intermediate steps: Completed HA bar calculated for {resampler_key}.") # DEBUG: Regression calculation intermediate steps

                if current_bar and ha_calc:
                    current_ha_bar = ha_calc.calculate_current_bar(current_bar)
                    logger.debug(f"Heikin Ashi calculation intermediate steps: Current HA bar calculated for {resampler_key}.") # DEBUG: Regression calculation intermediate steps
                ...
                logger.error(f"Critical data processing error: Error processing tick in Heikin Ashi resampler {resampler_key}: {e}", exc_info=True) # ERROR: Critical data processing errors
                continue
        ...
        for websocket in list(group.connections):
            ...
            if payload_key in payloads:
                logger.debug(f"Individual WebSocket message forwarding: Sending Heikin Ashi update to client {websocket.client.host} for {conn_info.symbol}/{conn_info.interval}") # DEBUG: Individual WebSocket message forwarding
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    if isinstance(result, (WebSocketDisconnect, ConnectionClosed)):
                        logger.warning(f"WebSocket connection failures: Failed to send Heikin Ashi update to a client, connection already closed.") # ERROR: WebSocket connection failures
                    else:
                        logger.error(f"WebSocket connection failures: Error sending Heikin Ashi data to client: {result}", exc_info=False) # ERROR: WebSocket connection failures
    ...
    async def _cleanup_loop(self):
        ...
        while True:
            ...
            for key in to_remove:
                ...
                if group.redis_subscription:
                    await group.redis_subscription.unsubscribe()
                logger.info(f"Cleaned up unused Heikin Ashi subscription: {key}")

# FastAPI App
app = FastAPI(
    title="WebSocket Heikin Ashi Data Service",
    description="Service for live Heikin Ashi data via WebSocket",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup_event():
    logger.info("WebSocket Heikin Ashi Data Service starting up...")
    await connection_manager.start()
    logger.info("WebSocket Heikin Ashi Data Service startup complete.")

@app.on_event("shutdown")
async def shutdown_event():
    logger.info("WebSocket Heikin Ashi Data Service shutting down...")
    await connection_manager.stop()
    logger.info("WebSocket Heikin Ashi Data Service shutdown complete.")

# WebSocket Handler
async def websocket_handler(websocket: WebSocket, symbol: str, interval: str, timezone: str):
    """Handler for Heikin Ashi live data websockets."""
    ...
    try:
        ...
        logger.info(f"Heikin Ashi client disconnected gracefully: {symbol}/{interval}")
    except Exception as e:
        logger.error(f"Error in Heikin Ashi websocket handler for {symbol}: {e}", exc_info=True)
    finally:
        await connection_manager.remove_connection(websocket)
        logger.info(f"Cleaned up Heikin Ashi connection for: {symbol}/{interval}")
...
async def get_live_heikin_ashi_data(
    ...
    """WebSocket endpoint for live Heikin Ashi data."""
    await websocket_handler(websocket, symbol, interval, timezone)

# Health Check
@app.get("/health", tags=["Health"])
async def health_check():
    """WebSocket Heikin Ashi Data Service health check."""
    ...
    return {
        ...
        "service": "websocket_heikin_ashi_data",
...
async def get_metrics():
    """Get detailed metrics for the Heikin Ashi WebSocket service."""
    groups_info = {}
    for channel, group in connection_manager.subscription_groups.items():
        groups_info[channel] = {
            ...
            "heikin_ashi_calculator_count": len(group.heikin_ashi_calculators),
