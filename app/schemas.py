...
class LiveRegressionResult(BaseModel):
    """Schema for a single live regression result."""
    slope: float = Field(..., description="The slope of the regression line")
...
class LiveRegressionUpdate(BaseModel):
    """Schema for live regression WebSocket updates with timeframe support."""
    type: str = Field("live_regression_update", description="Message type identifier")
    symbol: str = Field(..., description="Trading symbol")
    timeframe: str = Field(..., description="Specific timeframe for this update")
    context: str = Field(..., description="Context identifier (symbol:timeframe)")
    results: Dict[str, LiveRegressionResult] = Field(..., description="Regression results by lookback period")
    timestamp: str = Field(..., description="ISO timestamp of the update")

class LiveRegressionSubscriptionRequest(BaseModel):
    """Schema for live regression subscription parameters with multi-timeframe support."""
    symbol: str = Field(..., description="Trading symbol")
    exchange: str = Field(..., description="Exchange name")
    timeframes: List[Interval] = Field(..., description="List of time intervals for candles")
    timezone: str = Field("UTC", description="Timezone for calculations")
    regression_length: int = Field(..., ge=2, le=500, description="Number of candles for regression")
    lookback_periods: List[int] = Field(..., description="List of lookback periods")

class LiveRegressionStatus(BaseModel):
    """Schema for live regression service status with multi-timeframe details."""
    status: str = Field(..., description="Service status")
    active_subscriptions: int = Field(..., description="Number of active WebSocket subscriptions")
    active_calculation_contexts: int = Field(..., description="Number of active calculation contexts")
    active_redis_subscriptions: int = Field(..., description="Number of active Redis subscriptions")
    contexts: List[str] = Field(..., description="List of active context keys")
    contexts_by_symbol: Dict[str, List[str]] = Field(..., description="Active timeframes grouped by symbol")

class LiveRegressionSubscriptionConfirmed(BaseModel):
    """Schema for subscription confirmation message."""
    type: str = Field("subscription_confirmed", description="Message type identifier")
    symbol: str = Field(..., description="Trading symbol")
    exchange: str = Field(..., description="Exchange name")
    timeframes: List[str] = Field(..., description="Confirmed timeframes")
    regression_length: int = Field(..., description="Regression length")
    lookback_periods: List[int] = Field(..., description="Lookback periods")
    timezone: str = Field(..., description="Timezone")
    timestamp: str = Field(..., description="ISO timestamp")

class LiveRegressionError(BaseModel):
    """Schema for live regression error messages."""
    type: str = Field("error", description="Message type identifier")
